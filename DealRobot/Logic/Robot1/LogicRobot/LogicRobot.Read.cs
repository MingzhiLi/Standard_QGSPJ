...
namespace DealRobot
{
    partial class LogicRobot
    {
        ...
        public bool BlRequestPickPos = false;
        public bool BlRequestPlatHeightCom = false;

        public bool BlRequestPickOffset = false;

        public bool BlRequestPrecisePos = false;
        public bool BlRequestDumpPos = false;
        public bool BlRequestSpeed = false;
        #endregion 定义
        ...
        #region 东芝-请求数据
        public event Action SendPickOffset_event;
        public event Action SendPlatHeightCom_event;
        public event Action SendPrecisePos_event;
        public event Action SendDumpPos_event;
        public event Action SendSpeed_event;
        public event Action SendShakeHand;
        #endregion
        ...
        public event StrAction ShowState;
        public event StrAction ShowAlarm;
        #endregion 定义事件
        ...
        public void ReadData()
        {
            while (ParLogicRobot.P_I.blStartRead)
            {
                try
                {
                    ...
                    if (str == "Error")
                    {
                        //ShowAlarm("收到机器人信号：" + str);
                        return;
                    }
                    //ShowState("收到机器人信号：" + str);
                    TrrigerRobot(str);
                }
                catch (Exception ex)
                {
        ...
        public void TrrigerRobot(string str)
        {
            try
            {
                ...
                switch (Head)
                {
                    #region 东芝-请求数据
                    case "11":
                        BlRequestPickPos = true;  ///请求粗定位取料位置
                        break;
                    case "14":                        
                        SendPickOffset_event();
                        break;
                    case "26":
                        SendPlatHeightCom_event();
                        break;
                    case "500":
                        SendPrecisePos_event();
                        break;
                    case "501":
                        SendDumpPos_event();
                        break;
