    ...
    partial class WinInitMain
    {
        ...
        protected void LoginEvent_Robot()
        {
            try
            {
                ...
                //配置参数
                LogicRobot.L_I.ConfigRobot_event += new Action(LogicRobot_Inst_ConfigRobot_event);
                ...
                #region 东芝-请求数据
                LogicRobot.L_I.SendPrecisePos_event += new Action(SendPrecisePos);
                LogicRobot.L_I.SendPlatHeightCom_event += new Action(SendPlatHeightCom);
                LogicRobot.L_I.SendPickOffset_event += new Action(SendPickOffset);
                LogicRobot.L_I.SendDumpPos_event += new Action(SendDumpPos);
                LogicRobot.L_I.SendSpeed_event += new Action(SendSpeed);
                LogicRobot.L_I.SendShakeHand += new Action(SendShakeHand);
                #endregion

                LogicRobot.L_I.ShowAlarm += new StrAction(ShowAlarm_Robot);
                LogicRobot.L_I.ShowState += new StrAction(ShowState_Robot);
            }
            catch (Exception ex)
            {
        ...
        public void SetRobotNullRun()
        {
            try
            {
                ...
                {
                    LogicRobot.L_I.WriteRobotCMD(LogicRobot.RbtCmdNullRun, ShowState);
                    g_blRobotNullRun = true;
                    ShowState("机器人进入空跑模式");
                }
                else if (g_blRobotNullRun)
                {
                    LogicRobot.L_I.WriteRobotCMD(LogicRobot.RbtCmdExitNullRun, ShowState);
                    ShowState("机器人退出空跑模式");
                }
            }
            catch (Exception ex)
        ...
        void L_I_Shakehand_event()
        {
            ...
            {
                g_blRobotShake = true;
                ShowState("收到机器人握手完成信号，开始写入机器人参数");
                LogicRobot_Inst_ConfigRobot_event();
            }
            catch (Exception ex)
            {
                Log.L_I.WriteError("MainWindow", ex);
        ...
        public virtual void LogicRobot_Inst_ConfigRobot_event()
        {
        }
        #endregion 信息显示
        ...
        public virtual void SendPlatHeightCom()
        {

        }
        public virtual void SendPickOffset()
        {
        }
        public virtual void SendDumpPos()
        {
        }
        public virtual void SendSpeed()
        {

        }

        public virtual void SendShakeHand()
        {
