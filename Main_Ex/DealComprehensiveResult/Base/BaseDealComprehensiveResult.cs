    ...
    public partial class BaseDealComprehensiveResult
    {
        ...
        public StateComprehensive_enum DealComprehensiveResultFun(TriggerSource_enum trigerSource_e, int i)
        {
            ...
            try
            {
                ...
                if (i > 100 && i < 1000)
                {
                    stateComprehensive_e = DealComprehensiveResultFun_CalibSelf(trigerSource_e, i, out htResult);
                }
                #endregion 校准触发
        ...
        /// <returns></returns>
        public virtual StateComprehensive_enum DealComprehensiveResultFun_Calib1(TriggerSource_enum trigerSource_e, out Hashtable htResult)
        {
            htResult = null;
            return StateComprehensive_enum.False;
        }
        ...
        /// <param name="htResult">结果哈希表</param>
        /// <returns></returns>
        public virtual StateComprehensive_enum DealComprehensiveResultFun_Calib2(TriggerSource_enum trigerSource_e, out Hashtable htResult)
        {
            htResult = null;
            return StateComprehensive_enum.False;
        }
        ...
        /// <param name="htResult">结果哈希表</param>
        /// <returns></returns>
        public virtual StateComprehensive_enum DealComprehensiveResultFun_Calib3(TriggerSource_enum trigerSource_e, out Hashtable htResult)
        {
            htResult = null;
            return StateComprehensive_enum.False;
        }
        ...
        /// <param name="htResult">结果哈希表</param>
        /// <returns></returns>
        public virtual StateComprehensive_enum DealComprehensiveResultFun_Calib4(TriggerSource_enum trigerSource_e, out Hashtable htResult)
        {
            htResult = null;
            return StateComprehensive_enum.False;
        }
        ...
        /// 图片存储目录
        /// </summary>
        public string PathRootImage
        {
            get
            {
                return Log.CreateDayFile(ParPathRoot.PathRoot + "软件运行记录\\图片记录\\" + ComConfigPar.C_I.NameModel + "\\");
        ...
        /// 日志存储目录
        /// </summary>
        public string PathRootLog
        {
            get
            {
                return Log.CreateDayFile(ParPathRoot.PathRoot + "软件运行记录\\运行日志\\" + ComConfigPar.C_I.NameModel + "\\");
            }
        }
        ...
        /// 保存bitmap图像并返回保存路径
        /// </summary>
        /// <param name="blResult">计算结果，影响存入OK或者NG路径</param>
        /// <param name="strType">图片路径名称</param>
        /// <returns>存储图片的路径</returns>
        protected string SaveBitmapImage(bool blResult, string strType = "")
        {
            string strResult = "OK";
            if (!blResult)
            {
            ...
            try
            {
                string strPath = Log.CreateHourFile(PathRootImage + "\\Camera" + g_NoCamera + "\\" + strType + strResult + "图片\\")
                                + DateTime.Now.Minute + "-" + DateTime.Now.Second + "-" + DateTime.Now.Millisecond;

                g_BaseDealComprehensive.SaveImageFullPath_CurBit(strPath);
                return strPath;
            }
            catch (Exception ex)
            {
        ...
        /// <param name="cellMatch">预定位模板单元格</param>
        /// <param name="mLine1">M直线1</param>
        /// <param name="mLine2">M直线2</param>
        /// <param name="stdAngle">标准M直线夹角</param>
        /// <param name="threshold">夹角偏差阈值</param>
        /// <param name="htResult">算法结果</param>
        /// <param name="pMark1">输出的直线交点1坐标</param>
        /// <param name="pMark2">输出的直线交点2坐标</param>
        /// <param name="errorInfo">错误信息，无错误则为Empty</param>
        /// <returns>是否结果有错误</returns>
        public bool CheckMLine2(int pos, string cellMatch, string mLine1, string mLine2, 
                              double stdAngle, double threshold, Hashtable htResult, 
                              Point2D pMark1, Point2D pMark2, out string errorInfo)
        {
            try
            {
                BaseResult resultDingwei = htResult[cellMatch] as BaseResult;
                if (!DealTypeResult(resultDingwei) || resultDingwei.Num == 0)
                {
                    errorInfo = "位置" + pos + "预定位匹配Mark识别 NG";
                    return false;
                }

                //求取mark，直线或者其他mark,如果是直线的话，进行夹角判断，计算产品参数过程中已经加入轴坐标运算
                ResultCrossLines resultMark1 = htResult[mLine1] as ResultCrossLines;
                ResultCrossLines resultMark2 = htResult[mLine2] as ResultCrossLines;

                if (resultMark1.Num == 0)
                {
                    errorInfo = string.Format("相机{0}位置{1}上直线交点识别失败", g_NoCamera, pos);
                    return false;
                }

                if (resultMark2.Num == 0)
                {
                    errorInfo = string.Format("相机{0}位置{1}上直线交点识别失败", g_NoCamera, pos);
                    return false;
                }

                if (Math.Abs(resultMark1.IncludedR_J - stdAngle) > threshold)
                {
                    errorInfo = string.Format("位置{0}上直线夹角{1}°，超出偏差阈值{2}", 
                        pos.ToString(), resultMark1.IncludedR_J.ToString("f3"), threshold.ToString());
                    return false;
                }

                if (Math.Abs(resultMark2.IncludedR_J - stdAngle) > threshold)
                {
                    errorInfo = string.Format("位置{0}下直线夹角{1}°，超出偏差阈值{2}",
                        pos.ToString(), resultMark2.IncludedR_J.ToString("f3"), threshold.ToString());
                    return false;
                }
                //坐标
                pMark1.DblValue1 = resultMark1.X;
                pMark1.DblValue2 = resultMark1.Y;
                ShowState("位置" + pos + "上直线Mark抓取 OK,夹角：" + resultMark1.IncludedR_J);

                pMark2.DblValue1 = resultMark2.X;
                pMark2.DblValue2 = resultMark2.Y;
                ShowState("位置" + pos + "上直线Mark抓取 OK,夹角：" + resultMark2.IncludedR_J);
                errorInfo = string.Empty;
                return true;
            }
            catch (Exception ex)
            {
                errorInfo = string.Format("位置{0}直线判断进入异常！", pos);
                Log.L_I.WriteError(NameClass, ex);
                return false;
            }
        }
