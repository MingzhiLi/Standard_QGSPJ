...
namespace Main
{
    public partial class BaseDealComprehensiveResult_Main
    {
        ...
        public StateComprehensive_enum CSTProcessing(
            TriggerSource_enum trigerSource_e, int index, int pos,string strCell, out Hashtable htResult)
        {
            //Thread.Sleep((int)ModelParams.adjCSTPhotoDelay);
            ...
            if (index == 1)
                return RecordTopKeel(strCell, htResult);
            else if (index == 2)
                return RecordBottomKeel(strCell, htResult);
            else if (index == 3)
                return CalcKeel(pos, strCell, htResult);
            else
            {
                htResult = null;
        ...
        /// <param name="htResult"></param>
        /// <returns></returns>
        public StateComprehensive_enum CalcKeel(int pos,string strCell, Hashtable htResult)
        {
            try
            {
                BaseResult result = htResult[strCell] as BaseResult;
        ...
        /// <param name="htResult"></param>
        /// <returns></returns>
        public StateComprehensive_enum RecordTopKeel(string strCell, Hashtable htResult)
        {
            try
            {
                BaseResult result = (BaseResult)htResult[strCell];
                //把算子基准值记录到基准值
        ...
        /// <param name="htResult"></param>
        /// <returns></returns>
        public StateComprehensive_enum RecordBottomKeel(string strCell,Hashtable htResult)
        {
            try
            {
                BaseResult result = (BaseResult)htResult[strCell];
                ...
                if (result.Num != 0)
                {
                    ...
                    {
                        return DealResult(2, "下龙骨拍照次数大于龙骨数量本次拍照无效");
                    }
